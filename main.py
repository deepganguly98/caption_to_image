...
def sampling(text_encoder, netG, dataloader, ixtoword, device):
    ...
    netG.load_state_dict(torch.load(model_dir, map_location = device))
    netG.to(device)
    netG.eval()
...
def gen_sample(text_encoder, netG, device, wordtoix, captions):
    """
    ...
    #  'A herd of black zebra standing on a field',
    #  'A herd of white zebra standing on a field',
    #  'A herd of brown zebra standing on a field']

    # captions = ['A snowy landscape']

    # captions = ['A cartoon of batman fighting crime','Batman fighting bad guys','Batman fighting evil']
...
if __name__ == "__main__":
    ...
    state_dict = torch.load(cfg.TEXT.DAMSM_NAME, map_location=torch.device('cpu'))
    ...
    state_dict = \
        torch.load(img_encoder_path, map_location=torch.device('cpu'))
        # map_location=lambda storage, loc: storage
    ...
    if cfg.B_VALIDATION:
        ...
        captions = []
        st.header("cap2img app")
        st.title("Caption to Image Generator")
        title = st.text_input('Please enter a caption')
        # st.write('The current movie title is', title)
        # str = input("Enter a caption : ")
